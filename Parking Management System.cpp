...
void loadVehicles() {
	...
	if(maxAddress>5){
    	...
    	cols = (maxAddress % MAX_COLS) + 1; //to convert 1d array into 2D array
	}
    ...
    rewind(fptr);//send pointer to the starting of file
    while (fscanf(fptr, "%d %s %s %d %d %ld", &tempVehicle.address, tempVehicle.type, tempVehicle.plateNumber, 
                  &tempVehicle.charges, &tempVehicle.duration, &tempVehicle.t) == 6) {
...
void displayParkingArea(int n) {
	...
	int leftPadding = (totalWidth - parkingAreaWidth) / 2;//centre for logic
	printf("\n\n");
